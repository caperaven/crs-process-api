...
pub fn date_diff_duration(date1: &str, date2: &str) -> Result<Duration, String> {
    ...
    Ok(if d1 < d2 {d2.signed_duration_since(d1)} else {d1.signed_duration_since(d2)})
}
...
pub fn date_diff_struct(date1: &str, date2: &str) -> Result<DateDiff, String> {
    let duration = match date_diff_duration(date1, date2) {
        Ok(d) => d,
        Err(e) => return Err(e)
    };
